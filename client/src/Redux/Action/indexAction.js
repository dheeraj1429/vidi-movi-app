...
export const getMovieInScroll = function (data) {
    return async function (dispatch) {
        try {
            const fetchMovies = await axios.get(`/index/get-movies-in-scroll/${data}`, headers);
            console.log(fetchMovies);
            dispatch({
                type: ACTION_TYPE.GET_MOVIES_IN_SCROLL,
                payload: fetchMovies && fetchMovies?.data,
            });
        } catch (err) {
            console.log(err);
        }
